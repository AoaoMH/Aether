...
const searchQuery = computed(() => searchContext?.searchQuery.value ?? '')
const isMatched = computed(() =>
  matchesSearchQuery(searchQuery.value, props.value, normalizedText.value),
)

const isHidden = computed(() => Boolean(searchContext) && !isMatched.value)
...
watch(
  () => [props.value, normalizedText.value, isMatched.value] as const,
  ([value, text, matched]) => {
