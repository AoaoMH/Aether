    ...
    >
      ...
      >
        <div class="relative">
          ...
          <Input
            ...
            class="h-8 rounded-md border-border/60 bg-background pl-8 text-xs"
            @keydown.stop
          />
        </div>
      </div>
      ...
      <div class="max-h-48 overflow-y-auto">
        <div
          ...
          :key="item.value"
          class="flex cursor-pointer items-center gap-2 px-3 py-1.5 text-xs hover:bg-muted/50"
          @click="toggle(item.value)"
        >
          <input
            type="checkbox"
            :checked="modelValue.includes(item.value)"
            class="h-4 w-4 cursor-pointer rounded border-border/60 bg-card/80 text-primary shadow-sm accent-primary"
            @click.stop
