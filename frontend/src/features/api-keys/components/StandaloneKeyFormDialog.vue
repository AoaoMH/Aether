        ...
        <div class="pr-6 space-y-4">
          ...
          >
            ...
            <Input
              ...
              @update:model-value="
                (v) =>
                  (form.initial_balance_usd =
                    parseNumberInput(v, { allowFloat: true }) ?? 10)
              "
            />
            <p class="text-xs text-muted-foreground">
          ...
          <div class="space-y-2">
            ...
            <p class="text-xs text-muted-foreground">
              {{
                form.expires_at
                  ? "到期后" +
                    (form.auto_delete_on_expiry ? "自动删除" : "仅禁用") +
                    "（当天 23:59 失效）"
                  : "留空表示永不过期"
              }}
            </p>
          ...
          <div class="space-y-2">
            ...
            <Input
              ...
              @update:model-value="
                (v) =>
                  (form.rate_limit = parseNumberInput(v, {
                    min: 1,
                    max: 10000,
                  }))
              "
...
import { Plus, SquarePen, Key, Shield, X } from "lucide-vue-next";
...
async function loadAccessRestrictionOptions() {
  try {
    ...
    allApiFormats.value =
      formatsData.formats?.map((f: { value: string }) => f.value) || [];
  } catch (err) {
